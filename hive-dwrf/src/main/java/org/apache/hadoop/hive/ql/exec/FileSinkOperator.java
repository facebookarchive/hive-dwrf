...
public class FileSinkOperator extends TerminalOperator<FileSinkDesc> implements
  ...
  private transient boolean statsCollectRawDataSizeFromRecordWriter = false;
    ...
    try {
      ...
      for (int idx = 0; idx < totalFiles; idx++) {
        ...
        fsp.outWriters[filesIdx] = HiveFileFormatUtils.getHiveRecordWriter(
            ...
            reporter);
        // If the record writer provides the raw data size, get it from there instead of the serde
        statsCollectRawDataSizeFromRecordWriter =
  ...
  public void processOp(Object row, int tag) throws HiveException {
    ...
    try {
      ...
      if (conf.isGatherStats()) {
        if (statsCollectRawDataSize && !statsCollectRawDataSizeFromRecordWriter) {
      ...
      if (conf.isGatherStats() && statsCollectRawDataSize &&
          statsCollectRawDataSizeFromRecordWriter) {

        SerDeStats stats = ((StatsProvidingRecordWriter)rowOutWriter).getStats();
        if (stats != null) {
          fpaths.stat.addToStat(StatsSetupConst.RAW_DATA_SIZE, stats.getRawDataSize());
        }
      }
