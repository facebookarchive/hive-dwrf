...
public class TestDynamicArray {
  ...
  public void testByteArray() throws Exception {
    ...
    assertTrue(dba.compare(val, 0, 0, 2, 1) < 0);
    val = new byte[]{3,42};
    assertTrue(dba.compare(val, 0, 1, 2, 0) > 0);
    assertTrue(dba.compare(val, 0, 1, 2, 1) > 0);
    assertEquals(0, dba.compare(val, 0, 1, 3, 1));
    assertTrue(dba.compare(val, 0, 1, 3, 2) < 0);
    assertTrue(dba.compare(val, 0, 2, 3, 1) > 0);
    val = new byte[256];
    ...
    assertTrue(dba.compare(val, 0, 1, 0, 1) > 0);
